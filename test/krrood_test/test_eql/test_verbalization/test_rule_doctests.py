...
def test_rule_docstring_examples_execute(module_name):
    """
    Each rule/form docstring's ``>>>`` example produces exactly the documented
    output.
    """
...
def test_doctest_discovery_includes_previously_uncovered_modules():
    """
    The auto-discovery must execute doctests the old hand-maintained list
    silently dropped — in particular ``grammar.conditions.placement`` — and
    must still cover the originally-curated modules.
    """
    previously_curated = {
        "grammar.terms.rules",
        "grammar.chain.rules",

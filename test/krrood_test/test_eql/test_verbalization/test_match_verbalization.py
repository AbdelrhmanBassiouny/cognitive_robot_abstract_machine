...
def test_domain_search_opens_with_find():
    """A match carrying a domain (a search over existing instances) opens with *"Find"*."""
    assert verbalize_expression(an(Position)(x=1, y=2).from_([])).startswith("Find")
...
def test_multiple_none_assignments_coordinate_under_has_no():
    ...
    text = verbalize_expression(
        an(_Widget)(color=None, size=None, owner=object())
    )
...
def test_concrete_object_reads_as_a_specific_type():
    ...
    text = verbalize_expression(
        an(_Widget)(owner=Position(x=1.0, y=2.0, z=3.0))
    )
...
def test_concrete_object_uses_declared_identifying_attributes():
    ...
    text = verbalize_expression(
        an(_Widget)(owner=_Coded(serial=7, name="x"))
    )
...
def test_compound_value_pulled_out_of_respectively_group():
    ...
    text = verbalize_expression(
        an(_Trio)(a=1.0, b=2.0, c=variable(float, [7.0, 8.0]))
    )

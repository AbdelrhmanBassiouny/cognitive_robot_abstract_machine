...
PLACES = [TABLE, LID]
"""
The two places, as the world holds them, for a statement that describes one of them.
"""
...
# %% a narrowed statement handed over keeps its narrowing


def test_a_relation_given_a_narrowed_statement_keeps_that_narrowing(
    sightings: list[Sighting],
):
    lid = a(Place).from_(PLACES)
    lid.where(lid.name == LID.name)
    statement = a(Sighting).from_(sightings)
...
def test_a_relation_given_a_statement_narrowed_by_its_pattern_keeps_that_narrowing(
    sightings: list[Sighting],
):
    lid = a(Place)(name=LID.name).from_(PLACES)
    statement = a(Sighting).from_(sightings)
...
def test_a_function_given_a_narrowed_statement_keeps_that_narrowing(
    sightings: list[Sighting],
):
    @symbolic_function
    def place_of(sighting: Sighting) -> str:
    ...
    on_the_lid = a(Sighting).from_(sightings)
    on_the_lid.where(on_the_lid.place == LID.name)
    statement = a(Sighting).from_(sightings)

    narrowed = statement.where(place_of(statement) == place_of(on_the_lid))

    assert narrowed.tolist() == [CUBE_ON_THE_LID]


def test_a_statement_handed_over_twice_states_its_narrowing_once(
    sightings: list[Sighting],
):
    lid = a(Place).from_(PLACES)
    lid.where(lid.name == LID.name)
    ...
    narrowed = statement.where(StandingOn(statement, lid), StandingOn(statement, lid))

    assert len(narrowed._symbolic_expression_._where_builder_.conditions) == 3


def test_a_narrowing_reached_through_another_handed_over_statement_is_kept(
    sightings: list[Sighting],
):
    @symbolic_function
    def place_of(sighting: Sighting) -> str:
    ...
    table = a(Place).from_(PLACES)
    table.where(table.name == TABLE.name)
    on_the_table = a(Sighting).from_(sightings)
    on_the_table.where(StandingOn(on_the_table, table))
    statement = a(Sighting).from_(sightings)

    narrowed = statement.where(place_of(statement) == place_of(on_the_table))

    assert narrowed.tolist() == [CUBE_ON_THE_TABLE]


def test_the_statement_a_query_is_about_states_its_own_narrowing_once(
    sightings: list[Sighting],
):
    statement = a(Sighting).from_(sightings)

    narrowed = statement.where(statement.label == "cube", StandingOn(statement, LID))

    assert len(narrowed._symbolic_expression_._where_builder_.conditions) == 2

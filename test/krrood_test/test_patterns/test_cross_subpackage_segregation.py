...
def test_base_mixin_generated(mixin_source):
    """A DelegatorForCrossSubpackageBase class must be emitted for the sibling-subpackage ancestor."""
    assert "class DelegatorForCrossSubpackageBase" in mixin_source


def test_shared_method_not_duplicated(mixin_source):
    """sub_method appears exactly once (in DelegatorForCrossSubpackageBase, not in each taker DelegatorFor)."""
...
def test_shared_field_in_base_mixin_only(mixin_source):
    ...
    assert "sub_field" in _method_names(classes["DelegatorForCrossSubpackageBase"])
    assert "sub_field" not in _method_names(classes["DelegatorForTakerP"])
    assert "sub_field" not in _method_names(classes["DelegatorForTakerQ"])


def test_single_base_mixin_per_base(mixin_source):
    """DelegatorForCrossSubpackageBase is defined exactly once."""
    assert mixin_source.count("class DelegatorForCrossSubpackageBase") == 1
...
def test_taker_only_methods_stay_in_taker(mixin_source):
    ...
    shared_methods = _method_names(classes["DelegatorForCrossSubpackageBase"])
...
def test_base_mixin_has_abstract_role_taker(mixin_source):
    """DelegatorForCrossSubpackageBase declares an abstract role_taker property."""
    classes = _classes(mixin_source)
...
def test_base_mixin_emitted_before_taker_rolefors(mixin_source):
    """DelegatorForCrossSubpackageBase appears before DelegatorForTakerP in the source."""
    assert mixin_source.index("class DelegatorForCrossSubpackageBase") < mixin_source.index(
        "class DelegatorForTakerP"
    )

...
Step 8 — ``function_to_dataclass_source`` executes to a valid dataclass:
    ``TestFunctionToDataclassSourceExecutable`` – exec() produces a real dataclass;
    fields match; constructor call succeeds.
"""
...
# ===========================================================================
# Step 8 — function_to_dataclass_source: exec() and dataclass validity
# ===========================================================================


class TestFunctionToDataclassSourceExecutable:
    """The emitted source can be exec()'d into a valid, working dataclass."""

    @pytest.fixture(scope="class")
    def exec_namespace(self, module_level_distance: Callable) -> dict:
        """
        Exec the generated source and return the resulting namespace.

        We inject the actual ``distance`` function into the namespace before
        exec so the ``ClassVar`` assignment succeeds even though ``my.module``
        is not a real importable module.  The exec is done in a fresh dict to
        avoid polluting any shared namespace.
        """
        source = function_to_dataclass_source(module_level_distance)

        # Provide ``distance`` in the exec namespace so the ClassVar assignment
        # works without needing ``my.module`` to be importable.
        ns: dict = {"distance": module_level_distance}
        exec(compile(source, "<generated>", "exec"), ns)
        return ns

    def test_exec_produces_a_class(self, exec_namespace: dict) -> None:
        """The exec'd namespace must contain a class named ``Distance``."""
        assert "Distance" in exec_namespace
        assert inspect.isclass(exec_namespace["Distance"])

    def test_exec_produces_a_dataclass(self, exec_namespace: dict) -> None:
        """The ``Distance`` class in the exec'd namespace must be a dataclass."""
        assert dataclasses.is_dataclass(exec_namespace["Distance"])

    def test_dataclass_has_x_field(self, exec_namespace: dict) -> None:
        """The dataclass must expose a field named ``x``."""
        field_names = {f.name for f in dataclasses.fields(exec_namespace["Distance"])}
        assert "x" in field_names

    def test_dataclass_has_y_field(self, exec_namespace: dict) -> None:
        """The dataclass must expose a field named ``y``."""
        field_names = {f.name for f in dataclasses.fields(exec_namespace["Distance"])}
        assert "y" in field_names

    def test_dataclass_has_output_field(self, exec_namespace: dict) -> None:
        """The dataclass must expose a field named ``_output``."""
        field_names = {f.name for f in dataclasses.fields(exec_namespace["Distance"])}
        assert "_output" in field_names

    def test_dataclass_does_not_have_self_field(self, exec_namespace: dict) -> None:
        """The dataclass must not have a field named ``self``."""
        field_names = {f.name for f in dataclasses.fields(exec_namespace["Distance"])}
        assert "self" not in field_names

    def test_constructor_call_succeeds(self, exec_namespace: dict) -> None:
        """Instantiating the dataclass with valid field values must not raise."""
        Distance = exec_namespace["Distance"]
        instance = Distance(x=1.0, y=2.0, _output=0.0)
        assert instance is not None

    def test_constructor_preserves_x_value(self, exec_namespace: dict) -> None:
        """The ``x`` field stores the value passed to the constructor."""
        Distance = exec_namespace["Distance"]
        instance = Distance(x=3.14, y=0.0, _output=0.0)
        assert instance.x == pytest.approx(3.14)

    def test_constructor_preserves_y_value(self, exec_namespace: dict) -> None:
        """The ``y`` field stores the value passed to the constructor."""
        Distance = exec_namespace["Distance"]
        instance = Distance(x=0.0, y=2.72, _output=0.0)
        assert instance.y == pytest.approx(2.72)

    def test_function_classvar_is_callable(self, exec_namespace: dict) -> None:
        """The ``function`` class variable must be callable."""
        Distance = exec_namespace["Distance"]
        assert callable(Distance.function)

    def test_x_field_type_is_float(self, exec_namespace: dict) -> None:
        """The ``x`` field must have ``float`` as its annotated type."""
        Distance = exec_namespace["Distance"]
        field_map = {f.name: f for f in dataclasses.fields(Distance)}
        assert field_map["x"].type in (float, "float")

    def test_y_field_type_is_float(self, exec_namespace: dict) -> None:
        """The ``y`` field must have ``float`` as its annotated type."""
        Distance = exec_namespace["Distance"]
        field_map = {f.name: f for f in dataclasses.fields(Distance)}
        assert field_map["y"].type in (float, "float")

    def test_output_field_type_is_float(self, exec_namespace: dict) -> None:
        """The ``_output`` field must carry ``float`` as its annotated type."""
        Distance = exec_namespace["Distance"]
        field_map = {f.name: f for f in dataclasses.fields(Distance)}
        assert field_map["_output"].type in (float, "float")

    def test_field_count_is_three(self, exec_namespace: dict) -> None:
        """Exactly three fields (``x``, ``y``, ``_output``) must be present."""
        Distance = exec_namespace["Distance"]
        assert len(dataclasses.fields(Distance)) == 3

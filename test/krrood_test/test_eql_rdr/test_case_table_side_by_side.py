...
class DistinctFieldAnimal:
    ...
    Used so tests can assert the *new* case's value and the *corner* case's value appear
    independently in the rendered output without guessing field positions.
    """
...
def test_render_cases_side_by_side_keeps_values_intact_on_a_narrow_terminal(
    monkeypatch,
):
    """
    ...
    (``ea``/``gl``/``e``) even though the column is already wide enough to hold the
    header.
    """
    monkeypatch.setattr(

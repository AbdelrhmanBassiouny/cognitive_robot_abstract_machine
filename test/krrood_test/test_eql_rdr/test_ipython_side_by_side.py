...
def _make_animal(
    ...
    *,
    ...
    Copied verbatim from ``test_corner_case_population.py`` so this file is self-
    contained without a cross-test import.
    """
    return Animal(
        name=name,
        hair=milk,
...
# %% no corner case => output matches standalone render_case_table
...
# %% corner_case set => output contains both labels


def test_ipython_case_table_with_corner_case_contains_both_labels():
    """
    With ``context.corner_case`` populated, ``_case_table`` output contains both the
    new-case label and the corner-case label (side-by-side render was used).
    """
    rdr = EQLSingleClassRDR(Animal, "species")

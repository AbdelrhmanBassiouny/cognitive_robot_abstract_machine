"""
Tests for on-demand model save: ``ExpertInterface.save()``, ``_make_save_magic``, and
the auto-injection of ``on_save`` by ``EQLSingleClassRDR.fit()``.
"""
...
class TestMakeSaveMagic(unittest.TestCase):
    ...
    def test_magic_sees_late_injected_on_save(self):
        """
        The magic closes over the interface, so on_save injected after magic creation is
        visible.
        """
        called = []
        interface = FunctionInterface(answer_fn=lambda ctx, reqs: {})

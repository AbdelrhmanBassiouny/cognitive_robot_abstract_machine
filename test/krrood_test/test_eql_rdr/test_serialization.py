...
from krrood.entity_query_language.factories import add, entity, refinement, variable
...
class _SerializableRuleTree:
    ...
    def __init__(self, query, case_type, case_variable, conclusion_attribute_name):
        self.query = query
        self.case_type = case_type
        self.case_variable = case_variable
        self.conclusion_attribute_name = conclusion_attribute_name
        self.corner_cases = CornerCaseStore()
...
def test_generated_source_rebuilds_an_equivalent_rule_tree():
    ...
    assert namespace["RDR_CASE_TYPE"] is Animal
    assert namespace["RDR_CONCLUSION_ATTRIBUTE"] == "species"
    assert namespace["RDR_CORNER_CASES"] == {}
...
def test_generated_tree_has_the_same_backward_inference_knowledge_as_the_original():
    ...
    namespace = _exec_generated_module(source)
    rebuilt_root = namespace["RDR_QUERY"]._conditions_root_
...
def test_generated_tree_evaluates_a_bird_case_the_same_as_the_original():
    ...
    source = rdr_to_python(original)
    namespace = _exec_generated_module(source)
    rebuilt_variable = namespace["RDR_CASE_VARIABLE"]
    rebuilt_root = namespace["RDR_QUERY"]._conditions_root_
...
def test_raises_when_the_rdr_has_no_rules():
    ...
    with pytest.raises(ValueError, match="empty RDR"):
...
def test_recorded_corner_case_is_embedded_in_the_generated_source():
    ...
    assert corner_case in namespace["RDR_CORNER_CASES"].values()

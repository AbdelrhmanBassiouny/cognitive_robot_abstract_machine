...
``TestCorrectDrawerNoTargetFit`` runs automatically and tests the conclusion-asking flow
deterministically through both :class:`FunctionInterface` and stubbed
:class:`IPythonInterface` paths.

``TestCorrectLoadHumanFittedDrawerModel`` is NOT interactive: it runs automatically once
the human-authored model file exists, loading it and verifying full accuracy.
"""
...
def _labelling_expert(label_of):
    ...
    The ``answer_function`` is invoked twice per :meth:`Expert.ask_for_rule`:
    ...
    def answer(context, requests):
        """
        Answer whichever of the two questions this request is asking.
        """
        result = {
...
class TestCorrectDrawerNoTargetFit(unittest.TestCase):
    """
    Deterministic tests for the no-target conclusion-asking flow on the correct-drawer
    dataset.
    """
    ...
    def test_first_no_target_fit_case_classifies_correctly_and_unseen_returns_none(
        ...
        """
        Fitting the first case via ``ask_for_rule`` classifies it correctly; an unseen
        case returns ``None`` (no rule fires).
        """
        rdr = EQLSingleClassRDR(RDRTestCorrectDrawer, "correct")
        rdr.fit_case(
            drawers[0], target=..., expert=_labelling_expert(_drawer_ground_truth)
        )
        # The fitted case must classify to the expert-assigned conclusion.
        self.assertIs(rdr.classify(drawers[0]), True)
        # An unseen drawer has no matching rule → ... (the no-fire sentinel).
        self.assertIs(rdr.classify(drawers[1]), ...)
    ...
    def test_answer_fn_is_called_first_for_conclusion_then_for_conditions(self):
        """
        The ``answer_function`` receives the conclusion request first, then the
        conditions request.
        """
        call_log = []
        ...
        expert = Expert(interface=FunctionInterface(answer_function=tracking_answer))
        rdr = EQLSingleClassRDR(RDRTestCorrectDrawer, "correct")
        rdr.fit_case(drawers[0], target=..., expert=expert)

        # First call: conclusion only.  Second call: conditions only.
        self.assertEqual(len(call_log), 2)
        self.assertEqual(call_log[0], [AnswerName.CONCLUSION])
        self.assertEqual(call_log[1], [AnswerName.CONDITIONS])
    ...
    def test_stubbed_ipython_shell_drives_no_target_fit_through_ask_for_rule(self):
        ...
        def runner(namespace, header):
            """
            Answer whichever question the namespace is currently posing.
            """
            # The namespace contains the answer key for exactly one question:
            # ``AnswerName.CONCLUSION`` for the first, ``AnswerName.CONDITIONS`` for the second.
            if AnswerName.CONCLUSION in namespace:
                case = namespace[NamespaceName.CASE_INSTANCE]
                namespace[AnswerName.CONCLUSION] = _drawer_ground_truth(case)
...
class TestCorrectFitDrawerAsHumanExpert(unittest.TestCase):
    ...
    def test_fit_and_save(self):
        """
        Fit the drawers interactively, then save the learned tree.
        """
        rdr = EQLSingleClassRDR(RDRTestCorrectDrawer, "correct")
...
class TestCorrectLoadHumanFittedDrawerModel(unittest.TestCase):
    ...
    def test_loaded_model_classifies_correctly(self):
        """
        The saved drawer tree reloads and reproduces its fitted accuracy.
        """

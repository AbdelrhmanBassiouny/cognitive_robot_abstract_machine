...
class MaximallySpecificExpert(Expert):
    """Returns a rule matching the case's full feature vector.
    ...
    def ask_for_conditions(
        self, case, current_conclusion, target_conclusion, case_variable
    ) -> SymbolicExpression:
        return and_(
...
class ScriptedExpert(Expert):
    """Returns conditions from a per-(target) callable, for controlled scenarios."""
    ...
    def ask_for_conditions(
        ...
        self.calls.append((case.name, current_conclusion, target_conclusion))
        return self.rules[target_conclusion](case_variable)
...
class TestEQLSingleClassRDR(unittest.TestCase):
    ...
    def test_first_rule_via_fit(self):
        rdr = EQLSingleClassRDR(Animal, "species")
        expert = ScriptedExpert({Species.mammal: lambda v: v.milk == True})
        ...
        self.assertEqual(expert.calls[0][1], None)
    ...
    def test_no_fire_routes_to_alternative(self):
        ...
        self.assertEqual(expert.calls[-1][1], None)
    ...
    def test_wrong_fire_routes_to_refinement(self):
        ...
        self.assertEqual(expert.calls[-1][1], Species.fish)

    def test_fit_idempotent_when_already_correct(self):
        rdr = EQLSingleClassRDR(Animal, "species")
        expert = ScriptedExpert({Species.mammal: lambda v: v.milk == True})
        rdr.fit_case(first(Species.mammal), Species.mammal, expert)
        calls_before = len(expert.calls)
        # Fitting an already-correct case must not ask the expert again.
        rdr.fit_case(first(Species.mammal), Species.mammal, expert)
        self.assertEqual(len(expert.calls), calls_before)

    def test_full_fit_memorises_training_set(self):
        rdr = EQLSingleClassRDR(Animal, "species")
        rdr.fit(animals, targets, MaximallySpecificExpert())

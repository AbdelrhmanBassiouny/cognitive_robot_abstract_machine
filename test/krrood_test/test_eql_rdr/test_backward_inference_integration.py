"""
Tests for the RDR's own backward-inference surface: the wrapper that reads the current
rule tree, and the cache invalidation that keeps it honest as fitting grows that tree.
...
# %% the wrapper reads the current tree


def test_an_empty_rule_tree_knows_nothing_about_any_conclusion():
    rdr = EQLSingleClassRDR(Animal, "species")

    molusc_sufficient_conditions = rdr.sufficient_conditions_for(Species.molusc)

    assert isinstance(molusc_sufficient_conditions, ConclusionSufficientConditionSets)
    assert not molusc_sufficient_conditions.is_satisfiable()
...
def test_fitting_makes_the_new_rules_conclusion_satisfiable():
    rdr = EQLSingleClassRDR(Animal, "species")
...
def test_a_second_fit_widens_the_guards_of_the_first_rules_conclusion():
    ...
    bird_conditions_before = rdr.sufficient_conditions_for(Species.bird)
    ...
    assert not bird_conditions_before.is_satisfiable()

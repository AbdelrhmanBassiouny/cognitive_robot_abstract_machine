...
def generated_case_type(distance_func):
    ...
    Executes the source produced by ``FunctionCaseGenerator`` in an isolated
    namespace so no filesystem write is needed here.
    ...
    source = FunctionCaseGenerator().generate(distance_func)

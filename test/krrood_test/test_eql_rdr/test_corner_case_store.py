...
class SpySerializer(CaseSerializer):
    ...
    Replaces the old ``spy_emit`` raw callable after the Option A migration. The
    ``received`` list accumulates each case instance in call order so tests can assert
...
def test_to_ordered_sources_only_includes_recorded_nodes():
    ...
    Uses ``TrivialSerializer`` (Option A: serializer stored on the store, not passed
    inline).
    """
...
def test_to_ordered_sources_calls_emit_with_the_case():
    ...
    Uses ``SpySerializer`` (Option A: serializer stored on the store, not passed
    inline).
    """
    spy = SpySerializer()

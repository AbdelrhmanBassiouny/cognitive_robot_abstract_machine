...
class TestFitZooAsHumanExpert(unittest.TestCase):
    """
    Fits the zoo dataset with a real human at the shell, saving the tree it learns.
    """

    def test_fit_and_save(self):
        """
        Fit interactively, then write the learned tree next to these tests.
        """
...
class TestLoadHumanFittedModel(unittest.TestCase):
    """
    Checks the saved human-authored tree still loads and still classifies as fitted.
    """

    def test_loaded_model_is_usable_as_a_fit_model(self):
        """
        A reloaded tree infers through the backend exactly as it classifies directly.
        """

...
class TestIPythonProgressBar:
    ...
    All tests patch ``tqdm.tqdm`` so no real progress bar is created. The mock lets us
    verify construction arguments and lifecycle calls.
    """

    # -- construction -------------------------------------------------------

    @patch("tqdm.tqdm")
    def test_start_creates_tqdm_bar(self, mock_tqdm: MagicMock) -> None:
        """
    ...
    def test_start_replaces_old_bar(self, mock_tqdm: pytest.MagicMock) -> None:
        """
        Calling start() a second time creates a fresh tqdm instance without closing the
        previous one (the old bar is simply replaced).
        """
        bar = IPythonProgressBar()
        bar.start(10)

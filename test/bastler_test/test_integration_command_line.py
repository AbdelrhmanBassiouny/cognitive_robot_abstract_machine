...
from bastler.maintenance_constants import CREDENTIAL_VARIABLES
...
def test_a_command_that_exists_is_reachable_from_the_command_line():
    ...
    """
    assert bastler.integration_build_commands.BuildCommand() in bastler.integration_commands.COMMANDS
...
def test_every_family_of_commands_is_one_the_registry_has_imported():
    ...
    """
    registry = Path(bastler.integration_commands.__file__)
    families = {
        module.stem
    ...
    imported = {
        line.split()[1].removeprefix("bastler.")
        for line in registry.read_text().splitlines()
        if line.startswith("from bastler.integration_")
    }

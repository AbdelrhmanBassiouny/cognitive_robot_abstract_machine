...
from bastler.maintenance_github import CandidatePullRequests, CheckRunRecord
...
def test_each_verdict_leaves_the_status_a_caller_acts_on(
    ...
    """
    assert bastler.integration_candidate_commands._verdict_exit_code(verdict) is expected
...
def test_every_verdict_is_mapped_to_a_status():
    ...
    """
    assert {
        bastler.integration_candidate_commands._verdict_exit_code(verdict)
...
@pytest.mark.parametrize(
    "command",
    [
        bastler.integration_build_commands.BuildCommand(),
        bastler.integration_candidate_commands.OpenCandidateCommand(),
        bastler.integration_candidate_commands.SettleCandidateCommand(),
    ],
)
...
def test_a_build_the_suite_turns_red_blocks_the_branch_that_turned_it():
    ...
    """
    assert (
        bastler.integration_build_commands.BlockBranchCommand().invoked_as

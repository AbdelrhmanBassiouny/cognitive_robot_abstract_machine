...
from bastler.build_site import SitePath
from bastler.maintenance_git_commands import GitCommandRunner
from bastler.publish_site import (
    DEFAULT_SITE_BRANCH,
    SiteFile,
    SitePublisher,
...
class PublishingClone:
    ...
    def __post_init__(self) -> None:
        ...
        """
        self.remote_git = GitCommandRunner(working_directory=self.remote)
...
@pytest.fixture
def publishing_clone(
    scratch_repository: ScratchRepository, tmp_path: Path
) -> PublishingClone:
    """
    A scratch clone with a bare remote to publish into, and the publisher between them.

    :param scratch_repository: An initialized scratch project root.
    :param tmp_path: pytest's per-test temporary directory.
    :return: The clone, ready to publish.
    """
    scratch_repository.write(REPOSITORY_FILE, "")
    scratch_repository.commit_everything("the clone's own history")
    remote = scratch_repository.add_work_remote()

    return PublishingClone(
        clone=scratch_repository.project_root,
        remote=remote,
        publisher=SitePublisher(
            git=GitCommandRunner(working_directory=scratch_repository.project_root),
            remote=str(remote),
            branch=DEFAULT_SITE_BRANCH,
        ),

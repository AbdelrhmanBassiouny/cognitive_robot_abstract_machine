...
from bastler.integration_constants import BUILD_BRANCH_FILTER
from bastler.integration_exit_codes import IntegrationExitCode
from bastler.integration_pipeline_commands import RefreshJobVariable, RefreshWorkflowInput
...
def test_the_scheduled_job_calls_the_rebuild_rather_than_restating_it():
    ...
    """
    assert (
        bastler.integration_pipeline_commands.RefreshCommand().invoked_as
...
def test_a_rebuild_the_published_pipeline_cannot_perform_is_asked_for_before_it_is_run():
    ...
    assert bastler.integration_pipeline_commands.RefreshCommand().invoked_as in asking.script

...
would only discover by failing: that it drives the modules it means to, at the paths
those modules actually have; that it fires on the events a dashboard's data changes
...
DRIVEN_MODULES = (
    PACKAGE_DIRECTORY / "build_site.py",
    PACKAGE_DIRECTORY / "pages_site.py",
    PACKAGE_DIRECTORY / "publish_site.py",
    PACKAGE_DIRECTORY / "pyproject.toml",
)
...
def test_it_republishes_when_the_renderer_itself_changes(workflow: WorkflowDocument):
    """
    A change to the package changes every page, and no pull request event would rebuild
    them.
    """
    watched = workflow.trigger(TriggerEvent.PUSH)[TriggerKey.PATHS]

    assert f"{PACKAGE_DIRECTORY.relative_to(REPOSITORY_ROOT)}/**" in watched
...
def test_it_builds_with_the_renderer_it_was_started_with(checkout):
    """The workflow and the package it drives are versioned together, so the checkout
...
def test_it_names_no_path_of_its_own(workflow: WorkflowDocument):
    ...
    module cannot leave it pointing at a path that no longer exists.
    """
    for constant in SHARED_PATH_CONSTANTS:
        assert constant in workflow.text
    for module in DRIVEN_MODULES:
        assert str(module.relative_to(REPOSITORY_ROOT)) not in workflow.text


def test_the_modules_it_drives_exist():

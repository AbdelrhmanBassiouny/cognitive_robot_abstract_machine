...
from bastler.integration_tooling import ToolingFilter
...
INTEGRATION_SCRIPT = Path(bastler.integration.__file__)
"""
...
def create_blocked_branch(
    ...
    return PullRequestStackTipOutcome(
        ...
        status=bastler.integration_tips.TipStatus.BLOCKED,
...
def a_recorded_resolution(checkout: ForkCheckout) -> None:
    ...
    """
    for setting in bastler.integration_constants.RERERE_SETTINGS:
...
def create_red_branch(
    ...
    return PullRequestStackTipOutcome(
        branch=branch,
        pull_request_number=0,
        status=bastler.integration_tips.TipStatus.CHECKS_FAILED,

...
import bastler.maintenance
import bastler.maintenance_commands
...
from bastler.maintenance_constants import (
...
class ForkCheckout:
    ...
    def install_package(self) -> None:
        ...
        ``python3 -m bastler.<module>`` the way it does in a real fork.
...
def test_a_branch_no_step_concludes_is_an_error_rather_than_a_silent_pass(
    ...
    """
    a_parent_and_child(fork_checkout)
    monkeypatch.setattr(bastler.maintenance_restack_procedure, "RESTACK_STEPS", ())

    with pytest.raises(
        bastler.maintenance_restack_procedure.RestackConcludedNothingError
    ) as raised:
        restack(
            a_stack(fork_checkout, the_board()),
            fork_checkout.git,

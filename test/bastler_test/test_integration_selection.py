...
from bastler.git_commands import GitCommandResult, ReferenceUpdate
...
from bastler.integration_selection import (
    select_for_build,
    stack_to_build,
    tips_of,
)
from bastler.integration_tips import PullRequestStackTipOutcome, TipStatus
from bastler.integration_block_record import BlockRecord, BlockStanding
...
def test_a_restacked_branch_s_checks_are_read_after_the_restack_moved_it(monkeypatch):
    ...
    annotate = bastler.integration_selection.BranchChecks.annotate
    monkeypatch.setattr(
        bastler.integration_selection, "restack", lambda *arguments: restacks.append(1)
    )
    monkeypatch.setattr(
        bastler.integration_selection.BranchChecks,
        "annotate",

...
def test_the_live_stack_fetches_the_bases_as_well_as_the_heads(
    ...
    monkeypatch.setattr(bastler.stack, "load_configuration", make_configuration)
    monkeypatch.setattr(
        bastler.stack, "load_board", lambda: a_board(("child", "landed-elsewhere"))
    )
    monkeypatch.setattr(
        bastler.stack, "fetch", lambda configuration, branches: fetched.append(branches)
    )
    monkeypatch.setattr(
        bastler.stack, "merged_predicate", lambda configuration: lambda name: False
    )
    monkeypatch.setattr(
        bastler.stack,
        "containment_predicate",

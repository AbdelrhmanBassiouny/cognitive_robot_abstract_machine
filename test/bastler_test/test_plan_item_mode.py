"""
Tests for bastler.plan_item_mode, which resolves how a plan-item skill starts an item.
...
import bastler.plan_item_mode
from bastler.plan_item_mode import (
    CommandLineOption,
    ExecutionMode,
    ExitCode,
    Location,
...
MODE_MODULE = bastler.plan_item_mode.__name__
"""
The import path the scratch layout's copy of the module under test is run by, read off
...
def mode_repository(scratch_repository: ScratchRepository) -> ScratchRepository:
    ...
    """
    scratch_repository.install_hook_scripts(*BASH_ENTRY_POINTS)
...
def run_mode(
    ...
    """
    Run the scratch layout's copy of the module under test with *arguments*.
    ...
    return PythonModuleRunner(
        project_root=repository.project_root, module_name=MODE_MODULE
    ).run(*arguments)
...
def test_each_refusal_carries_its_own_exit_code():
    codes = {
        ...
        bastler.plan_item_mode.SettingsWriteRefusedError: ExitCode.SETTINGS_WRITE_REFUSED,
    }

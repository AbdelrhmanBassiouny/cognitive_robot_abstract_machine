...
from basstler.maintenance_github import CandidatePullRequests, CheckRunRecord

import basstler.integration_build_commands
...
def test_each_verdict_leaves_the_status_a_caller_acts_on(
    ...
    """
    assert basstler.integration_candidate_commands._verdict_exit_code(verdict) is expected
...
def test_every_verdict_is_mapped_to_a_status():
    ...
    """
    assert {
        basstler.integration_candidate_commands._verdict_exit_code(verdict)
...
REFRESH_WORKFLOW = (
    REPOSITORY_ROOT / ".github" / "workflows" / "integration-refresh.yml"
)
"""
...
@pytest.mark.parametrize(
    "command",
    [
        basstler.integration_build_commands.BuildCommand(),
        basstler.integration_candidate_commands.OpenCandidateCommand(),
        basstler.integration_candidate_commands.SettleCandidateCommand(),
    ],
)
...
def test_a_build_the_suite_turns_red_blocks_the_branch_that_turned_it():
    ...
    """
    assert (
        basstler.integration_build_commands.BlockBranchCommand().invoked_as

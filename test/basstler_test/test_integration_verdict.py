...
from basstler.git_commands import GitCommandResult
...
import basstler.integration_candidate_commands
import basstler.integration_exit_codes
import basstler.integration_pass_record
import basstler.integration_pipeline
import basstler.integration_verdict
import basstler.tool_runner
...
from basstler.integration_pipeline_commands import RefreshCommand
from basstler.workflow_document import (
...
from basstler.integration_verdict import (
...
def test_each_verdict_leaves_the_status_a_caller_acts_on(
    ...
    """
    assert (
        basstler.integration_candidate_commands._verdict_exit_code(verdict, published=True)
        is expected
    )
...
def settle(
    ...
    basstler.integration_candidate_commands.SettleCandidateCommand().run(
        run,
...
def test_a_candidate_no_run_can_judge_is_closed_so_a_rebuild_can_replace_it():
    ...
    status = basstler.integration_candidate_commands.CloseCandidateCommand().run(
...
def test_the_rebuild_runs_the_suite_before_it_pushes_anything():
    ...
    """
    assert not any("--no-test" in str(flag) for flag in basstler.tool_runner.CommandLineFlag)
...
def test_the_module_that_records_the_check_timing_says_how_long_it_was():
    ...
    """
    recorded = Path(basstler.integration_verdict.__file__).read_text()
...
def test_no_other_module_restates_how_long_a_candidate_waits():
    ...
    restating = [
        ...
        if module.name != Path(basstler.integration_verdict.__file__).name
...
def test_every_design_the_timing_shapes_refers_to_the_record():
    ...
    """
    shaped_by_it = (
        basstler.integration_pipeline,
        basstler.integration_pass_record,
        basstler.integration_candidate_commands,
        basstler.integration_exit_codes,
    )

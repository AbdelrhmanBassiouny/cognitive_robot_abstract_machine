...
def test_a_command_that_exists_is_reachable_from_the_command_line():
    ...
    """
    assert basstler.integration_build_commands.BuildCommand() in basstler.integration_commands.COMMANDS
...
def test_every_family_of_commands_is_one_the_registry_has_imported():
    ...
    """
    registry = Path(basstler.integration_commands.__file__)
    families = {
        module.stem
    ...
    imported = {
        line.split()[1].removeprefix("basstler.")
        for line in registry.read_text().splitlines()
        if line.startswith("from basstler.integration_")
    }

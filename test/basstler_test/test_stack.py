...
STACK_MODULE = basstler.stack.__name__
"""
...
def test_the_live_stack_fetches_the_bases_as_well_as_the_heads(
    ...
    monkeypatch.setattr(basstler.stack, "load_configuration", make_configuration)
    monkeypatch.setattr(
        basstler.stack, "load_board", lambda: a_board(("child", "landed-elsewhere"))
    )
    monkeypatch.setattr(
        basstler.stack, "fetch", lambda configuration, branches: fetched.append(branches)
    )
    monkeypatch.setattr(
        basstler.stack, "merged_predicate", lambda configuration: lambda name: False
    )
    monkeypatch.setattr(
        basstler.stack,
        "containment_predicate",

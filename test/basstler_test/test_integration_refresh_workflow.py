...
from basstler.integration_constants import BUILD_BRANCH_FILTER
from basstler.integration_exit_codes import IntegrationExitCode
from basstler.integration_pipeline_commands import RefreshJobVariable, RefreshWorkflowInput
from basstler.tool_runner import CommandLineFlag
...
def test_the_scheduled_job_calls_the_rebuild_rather_than_restating_it():
    ...
    """
    assert (
        basstler.integration_pipeline_commands.RefreshCommand().invoked_as
...
def test_a_rebuild_the_published_pipeline_cannot_perform_is_asked_for_before_it_is_run():
    ...
    assert basstler.integration_pipeline_commands.RefreshCommand().invoked_as in asking.script

...
import basstler.integration_reproduction
from basstler.integration_reproduction import (
    REPRODUCTION_MARKER,
...
import basstler.integration_commands
from basstler.integration_candidate_commands import ClearFixedBreaksCommand
from basstler.integration_constants import ReportKey
from basstler.stack import DefaultLabel, LabelWrite

from .constants import DATASET_DIRECTORY, REPOSITORY_ROOT
...
PYTEST_INI = REPOSITORY_ROOT / "pytest.ini"
"""
This repository's pytest configuration, which has to register the marker.
"""

SUITE_DIRECTORY = DATASET_DIRECTORY.parent
"""
This suite's own directory, which is where a run of it stops looking for conftests.

The reproductions sit under it, so a run that did not stop here would load the
repository-root ``test/conftest.py`` and die importing the robotics stack - the same
reason the targeted job passes ``--confcutdir``.
"""

REPRODUCTIONS_DATASET = DATASET_DIRECTORY / "reproductions"
"""
...
# Deliberately not a package: these modules are run by path, each importing the one
# beside it that names the branch, so the directory has to be importable in its own
# right - here, and in the subprocesses that inherit this path below.
sys.path.insert(0, str(REPRODUCTIONS_DATASET))

from recorded_break import BREAKING_BRANCH  # noqa: E402

PLUGIN_MODULE = basstler.integration_reproduction.__name__
"""
The plugin ``pytest`` is asked to load, named off the module rather than retyped.
"""

WORKFLOW = REPOSITORY_ROOT / ".github" / "workflows" / "integration-checks.yml"
"""
...
CONTINUOUS_INTEGRATION_WORKFLOW = REPOSITORY_ROOT / ".github" / "workflows" / "ci.yml"
"""
The repository's own workflow, one job of which installs what the targeted job installs.
"""

REQUIREMENTS_INSTALL = "${BASSTLER_PACKAGE_DIRECTORY}"
"""
...
def test_the_command_is_reachable_by_the_name_the_targeted_job_invokes_it_by():
    ...
    assert CLEAR_FIXED_BREAKS_COMMAND.invoked_as in {
        command.invoked_as for command in basstler.integration_commands.COMMANDS
    }

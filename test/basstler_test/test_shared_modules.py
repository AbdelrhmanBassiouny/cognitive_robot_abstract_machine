"""
Tests for the package modules that more than one tool imports.
...
import basstler.command_line
import basstler.dataclass_exception
import basstler.exceptions
import basstler.git_commands
import basstler.plan_model
from basstler.command_line import Command, commands_of
from basstler.dataclass_exception import DataclassException
from basstler.exceptions import ExternalCallFailed, GitCommandFailed
...
SHARED_MODULES = (
    basstler.command_line,
    basstler.dataclass_exception,
    basstler.exceptions,
    basstler.git_commands,
    basstler.plan_model,
)
"""
The modules under test, named individually because the package holds them beside every
other module rather than in a directory of their own.
"""
...
def test_every_shared_module_imports_with_nothing_installed():
    ...
    for module in SHARED_MODULES:
        ...
        assert result.returncode == 0, f"{module.__name__}: {result.stderr}"

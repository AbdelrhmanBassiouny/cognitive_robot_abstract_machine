...
import basstler.integration_pipeline_commands
from basstler.integration_commands import COMMANDS
from basstler.integration_exit_codes import IntegrationExitCode
from basstler.integration_pipeline import A_BUILD_WAS_ASSEMBLED, RefreshPipeline
from basstler.tool_runner import (
    CommandLineFlag,
    CommandOutcome,
...
from basstler.integration_verdict import ChecksVerdict, VerdictReportKey
...
def a_block(blocked: str = A_BROKEN_TIP) -> CommandOutcome:
    ...
    return answered(
        ...
        {basstler.integration_constants.ReportKey.BLOCKED: blocked},
    )
...
def test_the_rebuild_itself_is_one_of_those_commands():
    ...
    """
    assert basstler.integration_pipeline_commands.RefreshCommand().invoked_as in {
...
def test_the_rebuild_reads_each_document_by_the_key_the_command_writes(
    ...
    """
    assert str(read) in {str(written) for written in basstler.integration_constants.ReportKey}

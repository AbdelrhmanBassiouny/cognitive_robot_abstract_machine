...
from basstler.stack import PullRequest, Stack

import basstler.integration_constants
from basstler.integration_assembly import build_integration
from basstler.integration_block_record import BlockStanding
from basstler.integration_report import IntegrationReport
from basstler.integration_selection import build_branch_name
from basstler.integration_tips import ReadmittedBranch, ResolutionProvenance, TipStatus
...
def test_a_build_is_named_so_it_can_coexist_with_the_pointer():
    ...
    assert not name.startswith(f"{basstler.integration_constants.POINTER_BRANCH}/")
...
def test_the_pointer_moves_to_the_build_that_finished(fork_checkout: ForkCheckout):
    ...
    assert fork_checkout.git.commit_at(
        basstler.integration_constants.POINTER_BRANCH

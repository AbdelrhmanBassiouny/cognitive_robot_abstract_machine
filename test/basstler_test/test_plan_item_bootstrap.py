...
def test_every_operation_is_reachable_by_the_word_it_names():
    ...
    assert {
        ...
        for word, subcommand in basstler.plan_item_bootstrap.SUBCOMMANDS.items()
    } == {
...
def test_the_parser_takes_each_registered_word(
    ...
    refusals = {
        ...
        for word in basstler.plan_item_bootstrap.SUBCOMMANDS
    }

    assert refusals == {word: 0 for word in basstler.plan_item_bootstrap.SUBCOMMANDS}
...
def test_a_word_wrapped_at_its_hyphen_is_written_whole():
    ...
    """
    written = basstler.plan_item_bootstrap.fold(
...
def test_a_suspended_hyphen_typed_on_one_line_keeps_its_space():
    ...
    """
    written = basstler.plan_item_bootstrap.fold("all network- and credential-free tests", "")

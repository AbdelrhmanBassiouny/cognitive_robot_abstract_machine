"""
Downloads a Panda MJCF scene's referenced meshes from ``mujoco_menagerie`` on first
use.
"""
...
class PandaMeshAssets:
    """
    The Panda meshes a scene needs, downloaded from ``mujoco_menagerie`` on first use.

    The meshes are several tens of megabytes, so they are fetched on demand rather
    than committed alongside the scene that references them.
    """
    ...
    def directory(self) -> Path:
        """
        Where the meshes belong, taken from the scene's own ``meshdir`` so the two
    ...
    def download_if_missing(self) -> Path:
        ...
        for filename in missing:
            ...
            # leave a truncated mesh that later runs take for complete.

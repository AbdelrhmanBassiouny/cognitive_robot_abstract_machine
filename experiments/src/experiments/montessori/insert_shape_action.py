...
class InsertMontessoriShapeAction(ActionDescription):
    ...
    def _move_to_reach(self, target: Body, target_pose_end_effector: Pose) -> PlanNode:
        ...
        # The map is planar, so a node is looked up by the floor-plane projection of
        # the pose rather than by its full position.
        ...
        # action's underspecified fields, so it also needs to know about the condition

...
from cramera.request_origin import OriginHeader, RequestOrigin
...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    server: ViewerServer
    """The server owning request synchronization."""
    ...
    def end_headers(self) -> None:
        ...
        origin = RequestOrigin(self.headers.get(OriginHeader.ORIGIN)).response_value()
        if origin is not None:
            self.send_header(OriginHeader.ALLOW_ORIGIN, origin)
            self.send_header(OriginHeader.VARY, OriginHeader.ORIGIN)
        super().end_headers()

    def _accept_origin(self) -> bool:
        """Reject browser requests originating outside this computer."""
        if RequestOrigin(self.headers.get(OriginHeader.ORIGIN)).is_allowed():
            return True
        self._send_error("origin is not allowed", 403)
        return False
    ...
    def do_POST(self) -> None:
        """
        Route read-only queries and recording storage requests.
        """
        if not self._accept_origin():
            return
        route = self.path.split("?")[0]
...
class ViewerServer(socketserver.ThreadingTCPServer):
    """Serve one viewer instance and serialize its query requests."""

    allow_reuse_address: ClassVar[bool] = True
    """Allow a stopped viewer to restart on its previous port."""

    query_lock: threading.Lock = field(default_factory=threading.Lock)
    """Protect query evaluation from concurrent HTTP requests."""

    def __init__(self, port: int) -> None:
        """Bind the viewer server to localhost.
...
def main(arguments: Optional[List[str]] = None) -> None:
    ...
    with make_server(port) as server:
        ...
        logger.info(
            "scene bundles: %s%s",
            scenes,
            "" if Path(scenes).is_dir() else "  (no recordings yet)",
        )

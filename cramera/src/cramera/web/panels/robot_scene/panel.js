 ...
 * generated by `cramera-live` from real coraplex demo runs. Also owns the
...
Panels.define('robot-scene', function (root, bus) {
  root.innerHTML =
    ...
    '      <button id="record-share-confirm" class="play-btn record" title="Save the trimmed episode to the shared scene directory configured by CRAMERA_SCENES">🌐 Save to shared directory</button>' +
  ...
  // Add one observed object to the scene (bundle load or live update).
  ...
  fetch(SCENES + 'index.json')
    ...
    .then(function (index) {
      ...
      if (!name) {
        if (statusEl) statusEl.textContent = 'No scene found — run cramera-live first.';
        return;
      }
  ...
  // %% click-to-inspect
  const inspectionRay = new THREE.Raycaster(), inspectionNdc = new THREE.Vector2();
  let clickX = 0, clickY = 0, clickArmed = false;
  let partClickCb = null;

  function pointerNdc(event) {
    const bounds = renderer.domElement.getBoundingClientRect();
    inspectionNdc.set(((event.clientX - bounds.left) / bounds.width) * 2 - 1,
      -((event.clientY - bounds.top) / bounds.height) * 2 + 1);
  }
  function inspectableMeshes() {
    const meshes = [];
    for (const name in objectMeshes) {
      objectMeshes[name].traverse(function (child) {
        if (child.isMesh) { child.userData.simObj = name; meshes.push(child); }
      });
    }
    if (marker.visible !== false) {
      marker.traverse(function (child) {
        if (child.isMesh) { child.userData.simMarker = true; meshes.push(child); }
      });
    }
    return meshes;
  }
  renderer.domElement.addEventListener('pointerdown', function (event) {
    ...
    clickX = event.clientX; clickY = event.clientY; clickArmed = true;
  });
  renderer.domElement.addEventListener('pointerup', function (event) {
    if (!clickArmed) return;
    clickArmed = false;
    if (Math.hypot(event.clientX - clickX, event.clientY - clickY) > 5) return;
    const identifier = classifyClick(event);
    if (identifier && partClickCb) partClickCb(identifier);
  });
  renderer.domElement.addEventListener('pointercancel', function () { clickArmed = false; });

  function classifyClick(e) {
    pointerNdc(e);
    inspectionRay.setFromCamera(inspectionNdc, camera);
    let hits = inspectionRay.intersectObjects(inspectableMeshes(), false);
    if (hits.length) {
    ...
    if (robotModel) {
      hits = inspectionRay.intersectObject(robotModel.obj, true);
  ...
  function setLive(on) {
    ...
    // recording/place-demo affordance with no meaning in the live view —

 ...
 * live updates never re-run the layout.
 *
 * window.GraphView.create(container, legend) makes one renderer bound to one
 * container, so several panels can each show a graph of their own.
 * ==========================================================================*/
(function () {
  'use strict';
  ...
  // plus EXECUTING, which is the viewer's own: the running node execution has
  // actually reached, as opposed to the ancestors it is running underneath.
  const STATUS_STYLE = {
    EXECUTING:   { c: '#ffffff', w: 26, text: 'executing now', legend: 'executing now' },
  ...
  const STATUS_LEGEND_ORDER = ['EXECUTING', 'RUNNING', 'SUCCEEDED', 'FAILED', 'PAUSE', 'INTERRUPTED', 'CREATED'];
  ...
  /* One graph rendered into one container: every panel showing a graph creates
     its own, so two of them on screen at once do not draw over each other.

     The panel hands its elements over on creation — a panel owns its DOM subtree,
     so this renderer must never look them up on the document itself. */
  function create(el, legendEl) {
    if (!el) throw new Error('GraphView.create(container, legend) needs a container');
  ...
  window.GraphView = { create: create };
})();

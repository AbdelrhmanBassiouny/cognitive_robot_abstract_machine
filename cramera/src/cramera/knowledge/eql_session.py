"""
What one recorded episode offers EQL queries: the variables they range over.
"""
...
class EqlSession:
    ...
    def domains(self) -> List[QueryDomain]:
        """
        The ready-made variables a query of this session may range over: the recorded
        episode, and the architecture the repository was scanned into.
        """
        return [
    ...
    def extra_names(self) -> Dict[str, Any]:
        """
        Further names a query may use: the spatial types its values carry, the gripper
        type an arm's queries reach through, and each domain's objects as a plain list.
        """
        return {
            "Point3": Point3,
            "Gripper": Gripper,
    ...
    def runner(self) -> EqlQueryRunner:
        """
        A runner over this episode's domains.
        """
        return EqlQueryRunner(domains=self.domains(), extra_names=self.extra_names())
    ...
    def run(self, code: str, limit: int = DEFAULT_ROW_LIMIT) -> RenderResult:
        ...
        """
        return self.runner().run(code, limit)

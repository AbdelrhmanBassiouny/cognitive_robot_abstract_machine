...
class RefactoredRoleTransformer:
    ...
    def _transform_one_module(
        ...
        analysis_ctx = AnalysisContext(
            ...
            already_covered_bases=all_delegatees,
    ...
    def _split_result(
        ...
        Original classes are those whose names come from the source module.
        New classes (DelegatorFor*, RoleFor*) go to the mixin.
        """
        if result_module is None:
            return original_source, ""

        # Determine which class names were in the original source
        original_tree = libcst.parse_module(original_source)
        original_class_names: set[str] = set()
        for stmt in original_tree.body:
            if isinstance(stmt, libcst.ClassDef):
                original_class_names.add(stmt.name.value)
        ...
        for stmt in result_module.body:
            if isinstance(stmt, libcst.ClassDef):
                ...
                # Everything else (imports, etc.) stays in the transformed original
                transformed_body.append(stmt)
        ...
        mixin_code = (
            result_module.with_changes(body=mixin_body).code
            if mixin_body
            else ""
        )

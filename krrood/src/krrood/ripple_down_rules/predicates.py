...
from krrood.ripple_down_rules.utils import is_iterable, get_full_class_name
...
class Predicate(TrackedObjectMixin, ABC):
    ...
    def _reset_state(cls) -> None:
        """Reset predicate-specific state."""
        cls.inferred_once = False
        cls.cached_results.clear()
    ...
    def infer(cls, *args, **kwargs):
        ...
        for parent, child in results:
            ...
            cls.add_edge(
                cls._class_graph_indices[get_class_unique_name(parent)],
                cls._class_graph_indices[get_class_unique_name(child)],
                cls.relation(),
            )

...
The actual computation is delegated to the backend selected by
:class:`~krrood.entity_query_language.configuration.EqlConfiguration` (Python by default, symbolic
math on request).
"""
...
from krrood.entity_query_language.configuration import EqlConfiguration
...
class ArithmeticOperation(
    ...
    def _operation_result_(self, child_result: OperationResult) -> OperationResult:
        ...
        value = EqlConfiguration().math_operations.apply(self.math_operator, operands)
...
class UnaryArithmeticOperation(UnaryExpression, CanBehaveLikeAVariable[T]):
    ...
    def _negated_value_(self, child_result: OperationResult) -> Any:
        ...
        return EqlConfiguration().math_operations.apply(MathOperator.NEGATE, (value,))

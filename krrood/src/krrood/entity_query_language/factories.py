...
def a(
    ...
    *,
    ...
    """
    Select all values satisfying the given description; the consonant-article sibling of :func:`an`.

    Identical behaviour to :py:func:`an` (it delegates). Kept as a real function rather than an
    ``a = an`` alias so its ``__name__`` is ``"a"``, which lets tools that key a namespace by
    ``__name__`` (e.g. the doctest harness) expose it under the name ``a``.
    ...
    :return: The applied quantifier or the constructed match.
    """
    return an(entity_, quantification, domain=domain, target_type=target_type)

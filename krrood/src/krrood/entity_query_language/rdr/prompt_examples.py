...
class AttributeRef:
    ...
    :param path: Dot-separated access path relative to the case variable (e.g.
        ``"handle.name"`` or ``"milk"``).
    :param literal: The value of the attribute as a Python literal string (e.g.
        ``"'left_handle'"`` or ``"True"``).
    """
...
def build_conditions_example(ctx: RenderContext) -> str:
    ...
    if ref is None:
        return f"e.g. %conditions {NamespaceName.CASE_VARIABLE}.some_attr == True"
    return f"e.g. %conditions {NamespaceName.CASE_VARIABLE}.{ref.path} == {ref.literal}"

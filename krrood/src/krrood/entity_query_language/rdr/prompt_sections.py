...
New prompt situations become new :class:`PromptSection` entries in
...
#: IPython line magic that re-displays task-specific helper output.
HELPER_MAGIC = "helper"
...
class RenderContext:
    ...
    :param is_first_prompt: True only on the very first
        :meth:`~ExpertInterface.interact` call of a session; used to suppress one-time
        hints on subsequent prompts.
    """
    ...
    def is_conclusion_request(self) -> bool:
        ...
        return any(r.name == AnswerName.CONCLUSION for r in self.requests)

    @property
    def is_conditions_request(self) -> bool:
        """:return: True when the interaction is asking for conditions."""
        return any(r.name == AnswerName.CONDITIONS for r in self.requests)
...
def _ground_truth_conclusion(ctx: RenderContext) -> List[str]:
    """
    State the conclusion the case is known to deserve.

    :param ctx: The render context for the current interaction.
...
def _current_conclusion_vs_target(ctx: RenderContext) -> List[str]:
    """
    State what the tree concludes today, coloured by whether it already matches the
    target.
...
def _no_rule_fired_known_target(ctx: RenderContext) -> List[str]:
    """
    Say that nothing fired and ask for a condition that would.
...
def _conflict_resolution(ctx: RenderContext) -> List[str]:
    """
    Name the condition that concluded wrongly and ask for one that separates this case
    from it.
...
def _labelling_has_current(ctx: RenderContext) -> List[str]:
    """
    Offer the standing conclusion for confirmation, so an unchanged case costs one
    keystroke.
...
def _labelling_fired_anchor(ctx: RenderContext) -> List[str]:
    """
    Name the condition the standing conclusion came from.
...
def _labelling_no_rule(ctx: RenderContext) -> List[str]:
    """
    Ask for a label outright, since nothing fired to suggest one.
...
def _allowed_values(ctx: RenderContext) -> List[str]:
    """
    Show what the conclusion may be — the values themselves when they enumerate, the
    type otherwise.
...
def _contextual_example(ctx: RenderContext) -> List[str]:
    """
    Show a copy-pasteable example of the answer being asked for.
...
def _help_hint(ctx: RenderContext) -> List[str]:
    """
    Point at the magics that explain the case, naming the helper magic only when a
    ...
    """
    magics = f"%{HELP_MAGIC}"
    if supporting_material_presenters(ctx.case):
        magics += f" / %{HELPER_MAGIC}"
    return [ctx.palette.hint(f"Type {magics} for help with this case.")]


def _auto_resolution_hint(ctx: RenderContext) -> List[str]:
    """
    Show the resolver-suggested condition and how to accept it unchanged.

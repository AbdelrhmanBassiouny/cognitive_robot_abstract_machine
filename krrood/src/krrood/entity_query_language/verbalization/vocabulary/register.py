...
class Register:
    ...
    *"Find"* / *"Generate"* by the description's specificity."""
    ...
    def opener_fragment(self, default: Directive) -> VerbalizationFragment:
        """:return: the fixed opener fragment, or *default* when none is set -- each assembler
        supplies its own shape-derived default (a match's ``GENERATE``, a query's ``FIND`` /
        ``REPORT``)."""
        return (self.fixed_opener or default).as_fragment()

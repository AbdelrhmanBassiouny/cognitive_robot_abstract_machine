...
print("example       :", domain.example_for(AnswerName.CONCLUSION))
```
...
`answer_function` receives a `CaseContext` and the list of `AnswerRequest` objects and
returns the answers as a plain dict, keyed by `AnswerName`.

On the conclusion-asking path the function is called **twice** per case:

1. With a single `AnswerName.CONCLUSION` request — the expert chooses the label.
2. With a single `AnswerName.CONDITIONS` request — the expert writes the EQL expression.
...
def answer(context, requests):
    ...
    if any(r.name == AnswerName.CONCLUSION for r in requests):
        answers[AnswerName.CONCLUSION] = label_map[context.case_instance.name]
    # Conditions step: what distinguishes this exhibit?
    if any(r.name == AnswerName.CONDITIONS for r in requests):
        case = context.case_instance
        # The shared EQL variable — build expressions over it, not over the case.
        case_variable = context.case_variable
        answers[AnswerName.CONDITIONS] = case_variable.material == case.material
    return answers
...
same conclusion-asking path for each case (each paired with `...` internally):
...
def reaffirming_answer(context, requests):
    ...
    if any(r.name == AnswerName.CONCLUSION for r in requests):
        ...
        answers[AnswerName.CONCLUSION] = rdr.classify(context.case_instance)
    return answers

reaffirm_expert = Expert(
    interface=FunctionInterface(answer_function=reaffirming_answer)
)
result = rdr.fit_case(EXHIBITS[0], expert=reaffirm_expert)
print("returned conclusion:", result)
print("expert asked       :", call_count["n"], "time(s) — conditions step was skipped")
```

The conditions step is skipped entirely when the expert keeps the current conclusion.

## Step 7 — Plugging In a `ConclusionHelper`

A `ConclusionHelper` enriches the labelling session. What a helper offers is declared
by the mixin it inherits: `ConclusionSupportPresenter` shows supporting material, and
`ConclusionSuggester` proposes a candidate conclusion that pre-seeds the answer. A
helper may be both.

Here is one that does both from a lightweight heuristic:
...
class MaterialHeuristic(ConclusionSupportPresenter, ConclusionSuggester):
...
Pass the helper to `Expert` and it is consulted before the conclusion question is shown:

```{code-cell} ipython3
helped_expert = Expert(
    interface=FunctionInterface(answer_function=answer),
    helpers=[MaterialHeuristic()],
)

rdr3 = EQLSingleClassRDR(Exhibit, "kind")
rdr3.fit(EXHIBITS, expert=helped_expert)
print("With helper:", all(rdr3.classify(e) == l for e, l in zip(EXHIBITS, LABELS)))
```

When the heuristic suggestion is a valid domain member the expert can accept it by
simply not overriding the `AnswerName.CONCLUSION` key — the pre-seeded default stands.
...
| Pre-seed with a heuristic | `Expert(helpers=[MyHelper()])` |
...
Type %help / %helper for help with this case.
...
The magics `%help`, `%show_tree`, and `%helper` are available throughout.
...
def _answer(context, requests):
    ...
    result = {}
    if any(r.name == AnswerName.CONCLUSION for r in requests):
        result[AnswerName.CONCLUSION] = _label_map[case.name]
    if any(r.name == AnswerName.CONDITIONS for r in requests):
        result[AnswerName.CONDITIONS] = case_variable.material == case.material
    return result

rdr_e2e = EQLSingleClassRDR(Exhibit, "kind")
rdr_e2e.fit(EXHIBITS, expert=Expert(interface=FunctionInterface(answer_function=_answer)))
...
- {py:class}`~krrood.entity_query_language.rdr.conclusion_helper.ConclusionHelper` — base class for pluggable

...
8.  **[Why-Questions Internals](why_questions.md)**: How the `why(...)` ask surface is designed — the `WhyQuery` construct, the `WhyExplainer` protocol, deferred resolution, registry dispatch to the causal grammar, and the provenance/JTMS/contrastive grounding.
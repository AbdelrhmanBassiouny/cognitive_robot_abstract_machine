...
referring expression
  The noun phrase chosen to identify an entity — indefinite, definite, determiner-distinguished
  (*"another …"* / *"a second …"*), or pronoun.
...
| `VARIABLE`    | *Robot*, *Employee* | cornflower blue |
...
Rules emit the *first-mention* form — a `NounPhrase` tagged with a `referent_id` (and `Definiteness` INDEFINITE, or UNIQUE for a *"the unique X"* `the(entity(...))` selection), with variable-rooted chains emitted as a `PossessiveChain`.  The discourse focus (which referent is the current subject, for *"its …"*) is **not** marked by rules; it is projected once from the shared plan read model by {py:class}`~krrood.entity_query_language.verbalization.rendering.discourse.DiscourseModel` (`DiscourseModel.from_expression(expression, microplan)`), and the coreference pass consults it.  The pass then, in output order:

* keeps the first mention of a referent (e.g. *"a Robot"*) and marks it introduced;
* stamps a determiner-distinguishing feature (`NounPhrase.alternative` / `.ordinal`) on a referent whose noun is shared with another, assigned lazily on first encounter (`_distinguished`, below);
* downgrades a repeat **singular** mention to definite, dropping the first-mention modifiers (*"the Robot"*; a UNIQUE *"the unique Robot"* likewise downgrades to a plain DEFINITE *"the Robot"*) while **preserving** any distinguishing feature (*"another Robot"* → *"the other Robot"*, *"a second Robot"* → *"the second Robot"*) — unlike a descriptive `pre_head` (a ranking qualifier), which is correctly dropped on repeat;
...
The build is therefore free of in-fold coreference mutation.  {py:class}`~krrood.entity_query_language.verbalization.microplanning.referring.ReferringExpressions` holds only the pre-computed head-noun map and a `seen` **set** of introduced referent ids — the latter solely to *seed* the pass across builds sharing one services bundle.  `head_noun_of(variable)` is the single source of the resolved-noun lookup; `noun_for_parts(variable)` builds on it for the first-mention `NounForm` (always `INDEFINITE`).

### Head-noun resolution and the distinguisher index

Built by `MicroplanningServices.from_expression(expression)`, which pre-scans the tree twice: once to enumerate every node's parent edges ({py:func}`~krrood.entity_query_language.verbalization.microplanning.referring._child_edges_by_id`), once to resolve each numberable referent's head noun ({py:func}`~krrood.entity_query_language.verbalization.microplanning.referring.operand_head_noun`) and group canonical referents by that noun.

A referent's own **type** is the default head noun whenever it is informative (a concrete class, not the bare `object` placeholder) — the category identifies the referent, regardless of which predicate field it happens to fill (`HasType(a_body, Apple)` reads *"a Body is of type Apple"*, never *"a variable is of type Apple"*).  Only when the type carries no information does field context apply, and only for a variable reachable through exactly one predicate field and nowhere else (a structural condition, computed from the edge map — not an occurrence heuristic): the field's declared `GrammarMetadata.display_name`, else the field name itself, else `"object"`.

Same-noun referents (after the `==`-equality union-find that collapses an identified pair to one canonical) form a group; a group of one carries no distinguisher, a group of two gets the pair determiner (`NounPhrase.alternative`), a larger group gets ordinals (`NounPhrase.ordinal`, 1-based, starting at the second member).  This grouping — {py:class}`~krrood.entity_query_language.verbalization.microplanning.referring.DistinguisherIndex` — is precomputed, but **assignment of which member gets which position is not**: the pre-scan's graph-traversal order does not guarantee the order referents are actually said in the rendered text (folding, coordination, and shared-subject reduction all rewrite the mention order), so `DistinguisherIndex.distinguisher_for` assigns positions lazily, the first time the coreference pass encounters each canonical, in document order.  This is why disambiguation is a coreference-pass responsibility (`_distinguished`), not a pre-scan one:

```
Robot         (single)             →  no distinguisher — "a Robot"
Apple, Apple  (pair, first/second) →  "an Apple" / "another Apple"
Task ×3       (group of three)     →  "a Task" / "a second Task" / "a third Task"
```
...
|---|---|
| {py:class}`~krrood.entity_query_language.verbalization.microplanning.referring.ReferringExpressions` (`context.refer`) | head-noun map + distinguisher index + introduced-referent `seen` set |

...
A hop whose attribute is a **relation** (a past participle + preposition, e.g. `assigned_to`) renders as a relative clause — *"the `<Type>` which `<owner>` is `<verb-phrase>`"* — instead of the genitive *"the `<attr>` of `<owner>`"*; the head is the hop's `_type_` and the verb phrase comes from the same {py:func}`~krrood.entity_query_language.verbalization.grammar.conditions.recognition.relational_verb_phrase` used by absence.  Detection is *injected* into `build_path_parts` (the detector is passed by `ChainPlanner` / the navigation forms, so `navigation_path` stays free of any grammar import), and the relational flag rides on the {py:class}`~krrood.entity_query_language.verbalization.navigation_path.PathStep` (`relation`) so every navigation surface — possessive, boolean prefix, and the deferred chain — renders it uniformly.  Keeping the owner the verb's subject means the meaning never flips for agentive relations (*"the Person which a Book is owned by"*).  (The pronominalised *"its …"* surface keeps the genitive form for now — the relative clause would need a nominative pronoun, which the coreference layer doesn't yet model.)

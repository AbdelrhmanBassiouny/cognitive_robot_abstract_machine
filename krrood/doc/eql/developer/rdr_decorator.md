...
5. **Strict separation of concerns** — file lifecycle (`RDRFileStore`), case
   construction (`FunctionCase` + `FunctionCaseGenerator`), and call interception
...
| `FunctionCaseGenerator` | Code generation — emit the `@dataclass` source | `code_generation/function_case.py` |
...
sequenceDiagram
    ...
    participant Gen as FunctionCaseGenerator
    ...
    alt file absent
        Store-->>Wrap: False
        Wrap->>Gen: FunctionCaseGenerator().generate(func)
...
- **File absent**: `FunctionCaseGenerator().generate` produces the `@dataclass` class
...
```python
# ── class-header section (FunctionCaseGenerator) ─────────────────────────────
...
## `FunctionCaseGenerator` — Code Generation

`FunctionCaseGenerator().generate(func)` in `krrood.code_generation.function_case`
...
3. **`try/except NameError` around `Cls.function = func`** — mirrors the ImportError
   ...
   raise `NameError`.
4. **Type imports for non-builtin annotations** — every custom type referenced by a
   parameter or the return annotation is imported at the top of the generated file,
   so the model file stays importable on its own.
...
print(FunctionCaseGenerator().generate(navigate))
```
...
- **Non-builtin annotations must be importable** — `FunctionCaseGenerator` emits an
  import line for every custom type referenced by an annotation, so those types must
  live in importable modules (not in `exec`'d strings or interactive namespaces) for
  the saved model file to load in a fresh process.
...
`FunctionCaseGenerator` carries a `base_class_fully_qualified_name` field.  Pass a
...
```python
source = FunctionCaseGenerator(
    base_class_fully_qualified_name="mypackage.rdr_base.AuditedFunctionCase",
).generate(my_func)
```
...
| Change the generated base class | `base_class_fully_qualified_name` field on `FunctionCaseGenerator` — `code_generation/function_case.py` |
| Customise file placement | Subclass `RDRFileStore._resolve_path` — `rdr/file_store.py` |
| Add shared methods to all generated types | Subclass `FunctionCase`; pass as `base_class_fully_qualified_name` |

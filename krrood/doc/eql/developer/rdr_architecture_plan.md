...
**Scope:** `krrood/src/krrood/entity_query_language/rdr/` (the new EQL-based RDR package). The legacy `krrood/ripple_down_rules` package is backward-compatibility only and out of scope except as a reference for feature parity (MCRDR, GRDR, interfaces).
...
- **KA workflow:** when the expert writes new logic in the IPython interface, the magic command captures it as a **named feature**, appends it to the feature module (the same auto-serialization pipeline that already writes rule trees to source, `rdr/serialization.py`), and it becomes available to all future rules.
...
|---|---|
| Feature registry / CaseView / memoization | new module in `krrood/src/krrood/entity_query_language/rdr/` (or EQL core if attribute resolution hooks live there) |
| Feature capture + AST-dedup in KA flow | the `rdr` IPython interface (`rdr/interactive.py`, magic commands in `rdr/magics.py`) |
| Concept-tree declaration, dependency graph, stratified/semi-naive fixpoint | the `rdr` engine — **note:** GRDR/MCRDR exist only in legacy `krrood/ripple_down_rules`; the new `rdr` package currently has SCRDR only (`rdr/single_class.py`), so MCRDR/GRDR porting is a prerequisite for this layer |
| Cornerstone regression on feature/concept edits | where cornerstone cases are stored/validated in the `rdr` package (`rdr/corner_case.py`) |
| `definition` classmethods + candidate generators | domain-model side, consuming the `rdr` API; serialization piggybacks on the existing rules-to-source pipeline (`rdr/serialization.py`) |

## 4. Phased Plan (proposal — refine against code)

1. **Phase 0 — Grounding:** explore `entity_query_language` (especially its `rdr` package); map how conditions are represented (AST availability?), how rule trees serialize to source, how cornerstone cases are stored, how the IPython magics capture expert input. Compare against legacy `ripple_down_rules` for MCRDR/GRDR design worth porting.
...
3. **Phase 2 — MCRDR + GRDR in the `rdr` package:** port/redesign from legacy, but with the dependency graph and monotonic-conclusions contract designed in from the start.
...
- Where should feature resolution hook in: EQL's attribute access layer, or an RDR-specific case wrapper?
...
- Migration story: does anything in legacy `ripple_down_rules` (interfaces, tree types) constrain the new API for backward compatibility, or is the new `rdr` package free to diverge?
...
1. "Read `krrood/doc/eql/developer/rdr_architecture_plan.md`. Then explore `krrood/src/krrood/entity_query_language` (especially its `rdr` package) and `krrood/ripple_down_rules`. For each item in Section 3, report what already exists, what conflicts, and what's missing. Do not write code yet — produce a grounded implementation plan for Phase 1."
2. "Show me how a rule condition is currently represented and serialized in `entity_query_language/rdr`, and whether we have access to its AST at capture time (needed for feature extraction and dedup)."
3. "Compare the legacy MCRDR/GRDR implementations with the new `rdr` package's SCRDR design and propose how MCRDR/GRDR should look in the new architecture with the dependency graph built in."

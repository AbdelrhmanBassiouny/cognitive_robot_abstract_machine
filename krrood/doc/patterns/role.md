...
- The new class changes the permanent, identifying properties of the original. A `GenericRoom` that becomes a `Kitchen` does
not affect any of the things that makes it a `GenericRoom`, such as its size or location. But an `egg` that is boiled, changes

...
class ToolingScript(StrEnum):
    """
    The entry points a rebuild drives, by the module each is run as.

    By module rather than by path: a module run by path puts its own directory on
    ``sys.path`` in place of the repository root, so its absolute imports of its
    siblings would not resolve.
    """

    INTEGRATION = "basstler.integration"
    """
    Builds, publishes and judges the integration branch.
    """

    MAINTENANCE = "basstler.maintenance"
    """
    ...
    @property
    def path(self) -> Path:
        """:return: The module's own file in this checkout, which is what a tree has to
        carry for a rebuild to run from it."""
        return REPOSITORY_ROOT / f"{str(self).replace('.', '/')}.py"
...
class SubprocessToolRunner(ToolRunner):
    ...
    def run(
        ...
        finished = subprocess.run(
            [sys.executable, "-m", str(script), subcommand, *arguments],

 ...
 * playback / live controls, the layers overlay and the vial position sliders.
 *
...
Panels.define('robot-scene', function (root, bus) {
  root.innerHTML =
    ...
    '  </span></div>' +
    '  <div class="vial-ctl" id="vial-ctl" style="display:none">' +
    '    <span class="vc-title">Vial pickup position</span>' +
    '    <label class="vc-axis">X <input type="range" id="vial-x"><input type="number" id="vial-x-num" step="0.01"> m</label>' +
    '    <label class="vc-axis">Y <input type="range" id="vial-y"><input type="number" id="vial-y-num" step="0.01"> m</label>' +
    '    <label class="vc-axis">Z <input type="range" id="vial-z"><input type="number" id="vial-z-num" step="0.01"> m</label>' +
    '    <button id="vial-reset" class="vc-reset" title="Back to the recorded pickup spot">reset</button>' +
    '  </div>' +
    '</div>';
  ...
  renderer.domElement.addEventListener('pointermove', function (e) {
    ...
    // ---- object drag: the object follows the cursor onto whatever surface it
    // points at (no relative screen mapping, so no artificial reach limit) ----
    if (dragTarget.name) {
  ...
  // tools/live_viz.py streams the executing world's joints/base/objects in the
  // exact trajectory frame format; while attached, the viewer renders those
  // instead of the recording. The button appears whenever a bridge responds.
  ...
  function tick() {
    ...
    if (models.length && clock.getElapsedTime() < 25) { upgradeMaterials(); needsRender = true; }
  ...
  window.RobotView = {
    ...
    // no-ops kept so the existing layer panel bindings don't error
    setLabelsAlways: function (on) {
      labelsOn = !!on;
      for (const k in objectLabels) objectLabels[k].visible = labelsOn;
      needsRender = true;
    },
    onVialReady: function () {}, onVialMoved: function () {},
  };
  ...
  // ---- vial position sliders (synced with dragging in 3D) --------------------
  (function () {
    const ctl = $('vial-ctl');
    const sx = $('vial-x'), nx = $('vial-x-num');
    const sy = $('vial-y'), ny = $('vial-y-num');
    const sz = $('vial-z'), nz = $('vial-z-num');
    const reset = $('vial-reset');
    ...
    [sx, sy, sz].forEach(function (s, i) {
      s.addEventListener('input', function () { [nx, ny, nz][i].value = s.value; apply(); });
    });

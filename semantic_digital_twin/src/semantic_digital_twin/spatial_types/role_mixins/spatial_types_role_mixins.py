...
if TYPE_CHECKING:
    ...
    from krrood.symbolic_math.symbolic_math import CompiledFunction, FloatVariable, GenericSymbolicType, Matrix, Scalar, VariableParameters, Vector
    from semantic_digital_twin.spatial_types.spatial_types import HomogeneousTransformationMatrix, Point3, Pose, Quaternion, RotationMatrix, SpatialType, ca, np
...
class DelegatorForSpatialType(ABC):
    ...
    def __copy__(self, memo) -> Self:
        return self.delegatee.__copy__(memo)
    ...
    def _ensure_consistent_frame(
        ...
        return self.delegatee._ensure_consistent_frame(spatial_objects)
    def _parse_optional_frame_from_json(
        self, data: Dict[str, Any], key: str, **kwargs
    ) -> Optional[KinematicStructureEntity]:
        return self.delegatee._parse_optional_frame_from_json(data, key, kwargs)
...
class DelegatorForPose(DelegatorForSpatialType, ABC):
    ...
    def second_order_total_derivative(
        ...
        return self.delegatee.second_order_total_derivative(variables, variables_dot, variables_ddot)
    def substitute(
        self,

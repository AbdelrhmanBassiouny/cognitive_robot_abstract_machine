...
Commands (run from the repo root; ``--help`` on any of them for its flags)::
    ...
    python -m bastler.stack status         # the whole stack: parent, state, drift
    python -m bastler.stack check          # would each branch merge cleanly onto its parent now?
    ...
    python -m bastler.stack pin-tooling    # copy this tool out of the working tree
    ...
    python -m bastler.stack promotion-link # the upstream compare-and-create URL for a branch
    python -m bastler.stack reparents      # children whose base has landed, and the base they need
    python -m bastler.stack landed         # open fork pull requests whose branch has landed
...
from bastler.github_links import GitHubLinks

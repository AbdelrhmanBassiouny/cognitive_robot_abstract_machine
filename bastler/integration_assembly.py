...
from bastler.integration_carried_pipeline import pipeline_carried_by
...
def build_integration(
    ...
    if invoking_branch == POINTER_BRANCH:
        ...
        # call assembled - the upstream base plus each tip, which is not guaranteed to
        # carry this checkout's own files even with the loss above refused, since the
        # base itself may never have carried them. Only the ref was meant to move; the
        # checkout's files were not, so they are put back here, detached at the commit
        # this call found them at.

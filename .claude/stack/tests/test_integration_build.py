...
def test_a_build_keeps_the_checkout_s_own_files_when_it_sits_on_the_pointer(
    ...
    it is exactly what hands :class:`maintenance.DetachedCheckout`'s reattachment the
    ...
    fork_checkout.run_git(
        "checkout", "--quiet", "-B", integration_constants.POINTER_BRANCH, UPSTREAM_BASE
    )
    ...
    fork_checkout.branch_from(ONLY_TIP, UPSTREAM_BASE)
    fork_checkout.run_git("checkout", "--quiet", integration_constants.POINTER_BRANCH)

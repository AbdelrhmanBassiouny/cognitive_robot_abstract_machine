...
4. Block the plan items that branch carries, in the same step rather than at the end of the pass -
   the item is blocked from the moment you conclude it, and everything downstream reads the manifest
   as truth in the meantime. Write the same conflicting files or failing check the comment names:
...
5. At the start of every restack pass, fetch each `needs-resolution` branch's `mergeable_state`
   ...
   branch normally unless `mergeable_state` is `dirty`, and skip it only while it is. Clearing the
   label withdraws the blocker with it, and republishes the same way:
...
**You write only the blockers you decided yourself** - step 4's `block` and `unblock`,
under `${MAINTENANCE_BLOCKER_OWNER}`, which is why they can be withdrawn again by the
same pass. A reparent, a promotion or a landed branch is reported and not written:
which status those imply is a reading rather than a mechanical fact, and a landed
branch is corrected to `done` by the dashboard refresh on its own.

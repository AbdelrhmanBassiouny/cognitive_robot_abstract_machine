...
def test_a_pull_request_reads_every_field_the_dashboards_need():
    """
...
def test_an_entry_is_read_back_by_the_dashboard_as_the_same_state():
    """
...
def test_a_closed_entry_carries_its_null_merge_timestamp_explicitly():
    ...
    entry = PullRequest.from_json(
        PullRequestDetail(7, state=PullRequestState.CLOSED).to_json()
...
def test_fetching_asks_for_open_and_closed_pull_requests_alike():
    """The listing must cover closed pull requests too - a merged item is classified
    from one."""
    api = RecordingApi(pages=[[PullRequestDetail(1).to_json()]])
...
def test_fetching_stops_at_the_first_short_page():
    """
    ...
    api = RecordingApi(
        pages=[
            [
                PullRequestDetail(number).to_json()
                for number in range(PULL_REQUESTS_PER_PAGE)
            ],
            [PullRequestDetail(1000).to_json()],
        ]
    )
...
def test_fetching_keys_pull_requests_by_their_number_as_a_string():
    """
    ``pr_data.json`` is keyed by the number as a string, which is how a manifest's item
    is looked up in it.
    """
    api = RecordingApi(pages=[[PullRequestDetail(42).to_json()]])
...
def test_an_unknown_state_is_rejected():
    ...
    something it is not.
    """
    detail = PullRequestDetail(1).to_json()

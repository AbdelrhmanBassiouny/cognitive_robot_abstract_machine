"""
The Pages workflow, parsed once into something with names.
...
Deliberately sized to what these tests check. ``.claude/stack/workflow_document.py``
on the ``integration-branch-ci-verdict`` branch models the same thing far more fully -
inputs, matrix fan-out, called jobs - and is the version to keep whenever the two meet;
the names here are its names so that merge is an adoption rather than a rename.
"""

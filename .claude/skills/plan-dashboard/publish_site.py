...
Usage:
    python3 publish_site.py --source <built site directory> --branch <branch> \\
...
@dataclass
class SiteSourceMissingError(PlanDashboardError):
    """
...
class SitePublisher:
    ...
    def _empty_worktree(self, worktree: Path, scratch_branch: str) -> GitCommandRunner:
        ...
        if self.git.attempt("fetch", self.remote, self.branch, "--quiet").succeeded:
            ...
            worktree_git = self.git.in_directory(worktree)
...
class PublishSiteOption(StrEnum):
    """
    ...
    SOURCE = "--source"
    """
    The built site directory to publish.
    """

    BRANCH = "--branch"
    """
    The branch whose whole content it becomes.
    """

    REMOTE = "--remote"
    """
    The remote to publish that branch to.
    """

    MESSAGE = "--message"
    """
    The publishing commit's message.
    """
...
def main() -> int:
    ...
    parser.add(PublishSiteOption.SOURCE, "The built site directory to publish")
    parser.add(
        PublishSiteOption.BRANCH,
        "The branch whose whole content the site becomes",
        default=DEFAULT_SITE_BRANCH,
    )
    parser.add(PublishSiteOption.REMOTE, "The remote to publish the branch to")
    parser.add(PublishSiteOption.MESSAGE, "The publishing commit's message")
    arguments = parser.parse()

    publisher = SitePublisher(
        git=GitCommandRunner(Path.cwd()),

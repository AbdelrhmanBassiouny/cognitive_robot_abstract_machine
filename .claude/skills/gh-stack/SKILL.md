...
> **Environment note:** this skill drives the `gh stack` extension for the real `gh`
> CLI. It only works in a session that has `gh` v2.0+ installed, authenticated, and
> the extension added (`gh extension install github/gh-stack`) — typically a local
> terminal Claude Code session on a developer machine. Claude Code Remote / web
> sessions in this repo use the GitHub MCP tools instead of a local `gh` binary, so
> this skill does not apply there; use the `mcp__github__*` tools for those sessions.
...
# 2. Write code for the first layer (auth), commit it
git add auth.go
...
# 3. When you're ready for a new concern, add the next branch
...
# 4. Add a third layer for frontend

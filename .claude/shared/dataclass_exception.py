...
Mirrors ``krrood``'s dataclass-exception idiom - abstract ``error_message``/
``suggest_correction`` a concrete subclass supplies, ``__str__`` composes - without
importing it, since everything here is reachable from ``SessionStart`` and depends on
the standard library alone. Generic rather than tied to any one failure's fields, so
every dataclass-based exception in ``.claude/`` can share it instead of restating the
composition.
"""

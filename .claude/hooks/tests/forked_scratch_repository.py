...
STACK_TOOL_NAME = "stack.py"
"""
The tool within it a hook runs to resolve that repository.
"""
...
"""
Where the personal notes the session-start hook reads sit on the notes branch.
"""
...
class ForkedScratchRepository:
    ...
    def stack_configuration_refusal(self) -> str:
        ...
        result = subprocess.run(
            ["python3", f".claude/stack/{STACK_TOOL_NAME}", "configuration"],
...
def install_stack_tooling(repository: ScratchRepository) -> None:
    """
    Copy the real stack tool into a scratch layout and write the committed defaults the
    hooks resolve the upstream repository from.

    :param repository: The clone to install into.
    """
    destination = repository.project_root / ".claude" / "stack"
    destination.mkdir(parents=True, exist_ok=True)
    shutil.copy(STACK_SOURCE_DIRECTORY / STACK_TOOL_NAME, destination / STACK_TOOL_NAME)
    (destination / "stack.toml").write_text(STACK_CONFIGURATION)

...
the `basstler` package, so the whole directory was renamed underneath this
branch. Resolved:
1. `changed_paths.py` and `maintenance_tooling_label.py` moved into `basstler/`,
   `test_tooling_label.py` into `test/basstler_test/`, all on `basstler.` imports.
2. Test reaches the root through `basstler.package_layout.REPOSITORY_ROOT`
...
5. `stack.toml`'s `tooling_paths` comment no longer calls `basstler/` a place an
   in-flight branch is moving the tooling to.
6. `scripts/format_docstrings.py` run over every touched file.

**Test state** `pytest test/basstler_test --confcutdir=test/basstler_test`:
930 passed, 5 failed. Base branch alone: 912 passed, the *same* 5 failed. All
five start a subprocess with an interpreter that has no `basstler` installed,
...
- `integration_test_command` in `basstler/stack.toml` still names

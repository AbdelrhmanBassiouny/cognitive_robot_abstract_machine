...
Update 2026-09-12 (lab, #335): corpus draft PR #335 (claude/catalogue-in-simulation-qv7mk2, 5 commits, based on bb1bd5095) is dirty against #265 head 14f1e3439 only on two import lines of questions/working_memory.py (Relation from #325 vs EntityQueryLanguageBackend from #335); resolved locally, merged tree compiles. It adds run_corpus (scenarios x {randomized, partial, nearly-ambiguous} x {none + every perturbation}, perturbation_step SETTLE, built scene, headless, then each episode asked the long-term set 5x), four cross-episode long-term questions, Question.backend + RecordedQuery.answered_predicates (latency table now has rows), and fixes ObjectsTheRobotMovedInTheEpisode duplicates (85 of 300 askings wrong). Corpus was recorded on the synthetic robot only; on #265 head it is 5x3x5=75 episodes, the look scenario never ran through it. Ordering hazard: the lab session's #328 moves resolve_lasting_database into results_database.py while #335 imports it from record_episode; whichever merges second must fix that import. Lab session has opened #328-#336 against #265 (bring-up fixes; #332-#334 marked ready).
Update 2026-09-13 (morning): #265 head 96ad79fc2 carries #335 and the lab's #328-#342. Open against it: #343 (ready, Tracy's table annotated), the laptop corpus stack #344 (meshes kept beside the artifacts) -> #346 (inertia tensor stored; lab database already migrated) -> #348 (corpus releases worlds, recalls once) -> #349 (episode identifiers read off rows; asking 12 min -> ~2 min per episode), and #345 (ground truth from the scene as set up; SceneAsSetUp, TrueAnswer, PersonAtTheConsole asks which pieces were placed; --rescore-working-memory; CI green). Test-merged stack + #345 onto #265: no conflicts, compiles. Consequences: episodes recorded before #344 cannot be read back (mesh paths gone), #345 adds columns, so the robot re-records into a fresh database on the merged head, and the corpus recorded before #345 has working-memory rows scored right by construction -> re-run the corpus on the merged head. Developer's new ask: make the framework figure's plan run end to end in MuJoCo (perception, imagined-world SupportedBy, probabilistic grasp, RDR hole, giskard), with a coraplex InsertAction instead of PlaceAction and the cube on the far side of the lid, then on Tracy. Survey: no backend router exists (capability() from #303 unused); RDRBackend exists in krrood, unused by experiments; no InsertAction in coraplex (only InsertMontessoriShapeAction in experiments = pick + place); SortingStep has PUT_DOWN, no INSERT. Prompts given: lab re-record, corpus re-run, F1 InsertAction, F2 router + demo, F3 robot transfer.
Update 2026-09-13 (evening): #265 head 7389fd41fa = everything through #349 + #350/#351/#354 + two lab commits fixing the real pickup demo (reach designator = MontessoriShape, collision avoidance off, gripper joint-state subscription at sensor-data QoS, slip watch survives a rejected re-close). Not in #265: corpus stack #352 (MultiSimCamera body column) -> #353 -> #356 (cards drawn as the run stood; corpus re-recorded, scoring + figures running on it; experiments CI red on 3 MuJoCo sorting tests), F1 #355 (InsertAction + cube on the lid; CI red, 16 failures in test_tracy_pickup_demo_mujoco: cube on lid not perceived, board 5 cm off), #357 (polymorphic enum domain, off main, green), F2 stage 1 #359 (match as operand, underspecified(); stacked on #355). #344/#346/#348/#349 still open though their commits are in #265. Fix coverage on the robot: only pickup_demo_real.py sorts on Tracy and it carries all four fixes; record_episode.py --execution real accepts only scene-stands-still (the other four scenarios are runs_on_the_robot=False and drive simulated fingers). pickup_demo_real records no motions (no ObserverMotionListener -> motions=[]), reads the scene truth from the twin (SceneAsSetUp.read_from) rather than the person, takes no perturbation. Gripper setpoint change the developer mentions is not on origin (FingerSetpoint.CLOSED 0.5 / prism 0.6 unchanged since Aug 28) -> probably uncommitted on the lab PC. Advice given: record stand-still (+ piece-shoved, target-hole-moved) via record_episode and sorting via pickup_demo_real on #265 head into the corpus session's database (schema current), never from #355/#359 (schema changes); parallel session R1 to give pickup_demo_real the motion listener, the person at the console and a perturbation, then re-record sorting.

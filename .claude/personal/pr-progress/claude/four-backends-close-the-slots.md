...
`experiments/scripts/framework_demo.py --execution simulated`, the episode scenario, the
figure's data JSON, the rebuilt PDF, one slow end-to-end MuJoCo test. None of it runs
here (no ROS, no `iai_tracy_description`, no MuJoCo), so CI is the only place it can be
answered. The figure's target slot also needs `.where(Admits(hole, shape))`: the hole
description alone never says which piece it is wanted for.

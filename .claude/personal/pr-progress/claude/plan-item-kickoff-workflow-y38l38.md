...
branch is stacked on, had been rebased onto the `basstler` package move, so this branch's
...
- Moved by hand what git had no counterpart to rename: `basstler/github_links.py` and
  `test/basstler_test/fixtures/promotion_summaries.json`.
- 692 tests pass in `test/basstler_test`; `scripts/format_docstrings.py` reports no change.
